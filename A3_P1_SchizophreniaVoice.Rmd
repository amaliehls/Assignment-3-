...
library(crqa); library(tseriesChaos); library(nonlinearTseries); library(SparseM); library(lme4); library(plyr); library(MuMIn)
```
...
#Models 
# 1+trial|Subjects allow the model to make different intercepts and slopes over time for each subject
Model_syll = lmer(nsyll ~ Diagnosis * trial + (1+trial|Subject), data = mergedata)

Model_pause = lmer(npause ~ Diagnosis * trial + (1+trial|Subject), data = mergedata)
