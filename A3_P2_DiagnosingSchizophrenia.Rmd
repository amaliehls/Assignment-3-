...
library(crqa); library(tseriesChaos); library(nonlinearTseries); library(SparseM); library(lme4); library(dplyr); library(lmerTest); library(MuMIn): library(caret)
...
```{r}
modelrange = glm(diagnosis~range,family = "binomial",data = df_features)
summary(modelrange)
```

Calculate the different performance measures (accuracy, sensitivity, specificity, PPV, NPV, ROC curve) on a logistic regression using the full dataset. Don't forget the random effects!
```{r}
modelrange2 = lme4::glmer(diagnosis~range + (1+trial|Subject)+(1 + diagnosis + trial|Subject), family = "binomial", data = df_features)
summary(modelrange2)
...
for (i in folds)
  ...
  error.test1[n]=Metrics::rmse(testD$diagnosis,pred)
