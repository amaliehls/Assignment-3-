...
df_features$uniquesubject = unique(df_features$Subject)
```
...
modelrange2 = lme4::glmer(diagnosis~range + (1+trial|Subject), family = "binomial", data = df_features)
...
trainD$Predictionslogodds = predict(Mtrain,trainD)
...
testD$Predictionslogodds = predict(Mtrain,testD)
...
test_sen[n]=train_matrix$byClass[1]
test_spec[n]=train_matrix$byClass[2]
test_ppv[n]=train_matrix$byClass[3]
test_npv[n]=train_matrix$byClass[4]
...
cvalid = function(i,data=x){
  folds=createFolds(unique(x$Subject),k=5)
